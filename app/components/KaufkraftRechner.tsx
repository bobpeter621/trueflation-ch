...
export default function KaufkraftRechner() {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:items-end">
        <label className="flex flex-col gap-1 text-sm">
          ...
          <input
            ...
            aria-label="Betrag in Schweizer Franken"
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            aria-label="Startjahr auswählen"
            className="tf-numeric"
            style={{
              padding: "var(--space-2) var(--space-3)",
              border: "1px solid var(--color-border)",
        ...
        <button
          ...
          aria-label="Rechner-Ergebnis-Link kopieren"
          style={{ minHeight: "2.75rem" }}
        >
          {linkCopied ? "Link kopiert ✓" : "Ergebnis-Link teilen"}
        </button>
      </div>
      ...
      <div
        role="group"
        aria-label="Darstellungsart wählen"
      ...
      <dl className="mt-6 flex flex-col gap-3 text-sm">
        <div>
          <dt className="font-medium">{formatChf(betrag)} aus {jahr} entsprechen heute (nach LIK)</dt>
          <dd className="tf-numeric text-xl mt-1" style={{ color: "var(--color-line-lik, #4b5f7a)" }}>
            {likResult != null ? formatChf(likResult) : "Daten werden geladen…"}
          </dd>
          {modus === "rate" && (
            <dd className="tf-numeric text-sm mt-1" style={{ color: "var(--color-text-secondary)" }}>
              {formatRate(likAnnualRatePercent)} im Schnitt seit {jahr}
            </dd>
          )}
        </div>

        <div>
          <dt className="font-medium">{formatChf(betrag)} aus {jahr} entsprechen heute (nach Trueflation)</dt>
          {trueflationExists ? (
            <>
              <dd className="tf-numeric text-xl mt-1" style={{ color: "var(--color-line-trueflation, #d1495b)" }}>
                {trueflationResult != null ? formatChf(trueflationResult) : "Daten werden geladen…"}
              </dd>
              {modus === "rate" && (
                <dd className="tf-numeric text-sm mt-1" style={{ color: "var(--color-text-secondary)" }}>
                  {formatRate(trueflationAnnualRatePercent)} im Schnitt seit {jahr}
                </dd>
              )}
            </>
        ...
        <div className="mt-2 pt-3" style={{ borderTop: "1px solid var(--color-border)" }}>
          <dt className="font-medium" style={{ color: "var(--color-text-secondary)" }}>
            Zum Vergleich: {formatChf(betrag)} aus {jahr}, stattdessen gehalten in…
          </dt>
              ...
              <span className="tf-numeric" style={{ color: "var(--color-overlay-gold, #b8860b)" }}>
                {goldResult.result != null
                  ...
                  : goldResult.availableFrom != null
                    ? `verfügbar erst ab ${goldResult.availableFrom}`
                    : "nicht verfügbar"}
              </span>
            </span>
            <span>
              ...
              <span className="tf-numeric" style={{ color: "var(--color-overlay-btc, #c26a00)" }}>
                {btcResult.result != null
                  ...
                  : btcResult.availableFrom != null
                    ? `verfügbar erst ab ${btcResult.availableFrom}`
                    : "nicht verfügbar"}
              </span>
            </span>
          </dd>
