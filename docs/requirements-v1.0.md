...
- **US 1.8:** Als System will ich zwischen "neuer Datenpunkt" und "nachträgliche Revision eines bestehenden Datenpunkts" unterscheiden, damit sich die Historie nicht stillschweigend verfälscht.
  ...
  *AC (LIK-Rebasierung — planbares Ereignis, kein Ausnahmefall, Betreiber-Ergänzung 25.08.2026):* Das BFS rebasiert den LIK periodisch auf ein neues Basisjahr (im verifizierten Endpunkt bereits 13-mal geschehen, zuletzt vermutlich in Vorbereitung auf 12.2025=100). Dabei bleibt die zugrundeliegende Kurve identisch, nur die Skalierung ändert sich — kein realer Sprung. Die Pipeline importiert und speichert nur EINE Basisreihe (siehe `config/sources.json`, `importedBasis`) und muss erkennen, wenn eine neuere Basis in der Quelle erscheint: dann wird umgestellt und die Historie neu verkettet (identisches Prinzip wie 2.2a), NICHT die alte Basis stur fortgeschrieben. Dieser Vorgang ist der Plausi-Prüfung als erwarteter Regelvorgang bekannt zu machen, analog zur KVPI-Revision oben.
