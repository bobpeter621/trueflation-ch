...
|---|---|---|
| Krankenkassenprämien | Addition als eigene Kategorie | **✅ V2 GELÖST (25.08.2026) — Quelle geändert, siehe unten.** BAG-Prämienindex (mittlere Prämie je versicherte Person), NICHT der BFS-KVPI-Teilindex — dieser existiert für die Grundversicherung nicht eigenständig. National aggregiert, jährlich, Basis 2010=100, via XLSX-Parsing automatisierbar |
...
**✅ V2 final gelöst (Betreiber, 25.08.2026) — Kernbefund, der die gesamte Fallback-Bewertung ändert:** Der KVPI-Teilindex Grundversicherung ist **keine eigenständige BFS-Messung**. Das BFS erhebt für die Grundversicherung nichts eigenes, sondern übernimmt 1:1 die BAG-Schätzung der Durchschnittsprämien-Entwicklung. O-Ton BFS-Methodik: *"Eine spezifische KVPI-Erhebung wird für den Grundversicherungsbereich nicht durchgeführt."* Konsequenz: Der Weg über BAG-Durchschnittsprämien ist **nicht der Fallback**, sondern die **Primärquelle**, eine Verarbeitungsstufe weiter oben als ursprünglich angenommen. Die Suche nach einem isolierten KVPI-Teilindex wurde eingestellt — er existiert nicht in der gesuchten Form.

**Quelle (verifiziert 25.08.2026):** BAG-Statistik der obligatorischen Krankenversicherung (KVSTAT), Tabelle 3.01 "Mittlere Prämien je versicherte Person nach Kanton ab 1996", Zeile "CH" (nationaler Durchschnitt). Bezogen über opendata.swiss → bag.admin.ch (jährliches KVSTAT-ZIP-Archiv). Live extrahiert: 29 Jahreswerte 1996–2024, lückenlos, CH 2024 = 4'278 CHF.

**Methodische Entscheidung (Betreiber, 25.08.2026, fixiert):** Verwendet wird die **mittlere Prämie pro versicherte Person** (alle Versicherten, inkl. Kinder) — **nicht** die Variante "Erwachsene ab 26 Jahren" (CH 2024 = 5'081 CHF). Begründung: Das HABE-Gewicht ist ein Haushaltsbudget-Anteil, und Haushalte schliessen Kinder ein; die Erwachsenen-Variante würde die Belastung überzeichnen. Indexreihe: Basisjahr **2010 = 100**, konsistent mit dem Trueflation-Start und dem ersten Basisjahr der Warenkorb-Fixierung (2.2a).

**kvpiDataUrl aus der LIK-App (aus V1-Fund):** Der Wert `korrekturFaktor: 27.38` ist **nicht** der Prämienindex — die Grössenordnung deutet auf einen internen Gewichtungsfaktor der LIK-App hin (evtl. für deren eigenen Indexierungsrechner). Diese Quelle bleibt in `config/sources.json` registriert (Status `unverified`), wird aber **nicht** für 2.2b verwendet.

**Bekannte Einschränkungen des BAG-Prämienindex (transparent auszuweisen, unverändert gültig):**
- **Prämienverbilligungen sind nicht berücksichtigt** — die Reihe zeigt Bruttoprämien. BFS schätzt selbst, dass deren Einbezug das ausgewiesene Prämienwachstum um rund 0,5 Prozentpunkte pro Jahr reduzieren würde. Muss auf der Methodik-Seite explizit stehen, sonst ist es der naheliegendste Angriffspunkt ("ihr rechnet mit Prämien, die viele so nicht zahlen").
- **Jahresaktualisierung im Herbst** (Prämiengenehmigung fürs Folgejahr) — kein monatlicher Rhythmus, passt zur in 2.2a beschriebenen jährlichen Aktualisierung der Korrekturfaktoren.
...
#### 2.2d Status-Übersicht der drei Trueflation-Komponenten (FINAL GEMESSEN, Betreiber-Entscheidung 28.08.2026)

**Einheitliches Kriterium (vorab festgelegt 28.08.2026, gilt für den BETRAG unabhängig vom Vorzeichen):** `|Effekt| >= 0.10 pp/Jahr UND Abdeckung >= 10 der 15 Jahre (2010-2024) -> als Overlay bauen, sonst -> als Befund dokumentieren.` Ein negativer Effekt ist genauso wertvoll wie ein positiver — er widerlegt eine Annahme und gehört gezeigt, nicht versteckt.

**Fixer Warenkorb: GEPRÜFT, ERGEBNIS DOKUMENTIERT (28.08.2026) — Kriterium NICHT erfüllt, kein Overlay.** Lösung des 12<->13-Zuordnungsproblems: die AKTUELLSTEN Gewichte (Tabelle "LIK-Warenkorb und Gewichte 2026", 13 Hauptgruppen, BFS-Themenseite `/bfs/de/home/statistiken/preise/erhebungen/lik/warenkorb.html`, live extrahiert 28.08.2026, siehe `config/lik-warenkorb-gewichte-2026.json`) wurden rückwärts über 2010-2024 fixiert und gegen dieselbe 13er-`posId`-Struktur verrechnet, die auch die verkettete Reihe (`mainIndex`) speist — kein Kategorien-Mismatch, keine Zuordnung zwischen unterschiedlichen Klassifikationen nötig (siehe `scripts/pipeline/build-warenkorb-fixation-test.mjs`).
- **Gemessener Effekt: -0.035 pp/Jahr** (fixiert p.a. 0.3276% vs. verkettet p.a. 0.3626%, 2010-01 bis 2024-12, geometrisch annualisiert).
- **Abdeckung: 15 von 15 Jahren** (voll, lückenlos).
- **Kriterium: NICHT erfüllt** (Betrag 0.035 < Schwelle 0.10 pp/Jahr) → als Befund dokumentiert, NICHT als Overlay gebaut. Der frühere Fund '2026er-Gewichte = 0.06' war ein geschätzter Platzhalterwert aus einer früheren Session, keine amtliche Quelle — durch die echte Tabelle ersetzt.
- Sensitivitätsvorbehalt: andere Fixierungsjahre (2010/2015/2020 statt 2026) würden das Ergebnis leicht verschieben, nicht die Grössenordnung — nicht separat berechnet (Zeitbudget).

**Miet-Korrektur: GEPRÜFT, ERGEBNIS DOKUMENTIERT (28.08.2026) — Kriterium NICHT erfüllt (Abdeckung), kein Overlay.** Zwei Korrekturen zum Vorzeichen/zur Bewertung wurden vorgenommen (Betreiber, 28.08.2026): (a) das negative Vorzeichen der Total-Variante ist PLAUSIBEL — der Referenzzinssatz wurde 2023 zweimal erhöht, wirkt auf Bestandsmieten, im Fenster 2020-2024 holen Bestandsmieten auf; (b) -0.165 pp/Jahr ist die Hälfte des Prämieneffekts, nach dem Kriterium erheblich, nicht vernachlässigbar — der Blocker ist allein die Abdeckung.
- **Vergleichsgruppen-Prüfung** (`scripts/pipeline/build-rent-correction-longtenure-check.mjs`): Vergleich gegen "21 Jahre und mehr" (längste Bezugsdauer-Klasse, enthält KEINE aktuellen Neubezüge, sauberere Referenz als "Total", das die Neubezüge bereits enthält) DREHT DAS VORZEICHEN: gegen Total -0.156 pp/Jahr, gegen 21J+ **+0.253 pp/Jahr**. Roh-Wachstumsraten 2020→2024: Total +8.16%, Neubezug-Proxy +7.50%, 21J+ +6.43%.
- **Abdeckungsprüfung** (strikt timeboxed, ~15 Min, siehe `data/rent-correction/coverage-check-28082026.json`): BFS-DAM-API nach älteren Jahrgängen der Bezugsdauer-Tabellen durchsucht (analog "Warenkorb und Gewichte - JJJJ"-Muster). Gefunden: drei ältere Jahrgänge (2021/2022/2023) existieren nur als OLD-Lifecycle-Assets, sind aber Diagramm-Dateien (`DIAG_DM01`), keine Rohdaten-Tabellen, und ihre `master`-Downloads liefern HTTP 404 (archiviert). **Kein Treffer** — kein Analogon zum Warenkorb-Muster für diese Kennzahl.
- **Abdeckung bleibt 2020-2024 (5 von 15 Jahren).**
- **Kriterium: NICHT erfüllt** — Betrag erfüllt die Schwelle in BEIDEN Vergleichsgruppen (|0.156| und |0.253| >= 0.10 pp/Jahr), aber die Abdeckung (5 von 15) erfüllt die 10-von-15-Schwelle nicht. → als Befund dokumentiert, NICHT als Overlay gebaut.
- **Meldepflichtiger Widerspruchsbefund bleibt bestehen:** Kreuzprüfung gegen die Mietpreis-PRO-M2-Tabelle zeigt weiterhin die gegenteilige Korrekturrichtung (+1.84pp statt -0.66pp gegen Total) — absoluter Mietpreis und Preis pro Quadratmeter widersprechen sich in der Richtung. Mögliche, nicht verifizierte Ursache: Neumieter beziehen im Schnitt kleinere Wohnungen.

**Strom: bleibt GESTRICHEN** (unverändert, siehe 2.2c — Doppelzählung belegt, Elektrizität bereits vollständig im LIK enthalten).

**Kernzahl (LIK 5.51% / Trueflation 9.66% Jahresdurchschnitt 2010-2024): VORLÄUFIG-Markierung aufgehoben (28.08.2026).** Beide offenen Komponenten (Warenkorb, Miete) sind jetzt geprüft und als Befund dokumentiert (nicht als Overlay umgesetzt) — die Kernzahl basiert bewusst nur auf LIK + Prämienkorrektur, das ist der abschliessende Scope für v1, kein Zwischenstand mehr. Warenkorb- und Miet-Effekte sind auf der Methodik-Seite mit den gemessenen Zahlen auszuweisen (P5-Content, siehe Abschnitt 8/13).
...
- **Quelle (verifiziert 25.08.2026, V6 zweiter Teil grösstenteils abgeschlossen):** ZWEI SNB-Cubes, nicht einer — gefunden über die SNB-Discovery-Heuristik (siehe Abschnitt 3), nicht durch Cube-Namen-Raten:
  - **`snbgwdzid`** ("Zinssätze und Faktor für die Limite"), Dimension `D0=LZ` — SNB-Leitzins **ab 13. Juni 2019** (live bestätigt: erster Wert -0.75 am 13.06.2019, lückenlos bis heute nachvollzogen).
  - **`snboffzisa`** ("Offizielle Zinssätze"), Dimension `D0=UG0` (untere Grenze SNB-Zielband für Dreimonats-Libor in CHF) — Libor-Zielband **von Januar 2000 bis Mai 2019** (live bestätigt: erster Wert 1.25 im Januar 2000, letzter Wert Mai 2019, danach leer). **Korrektur einer Fehlannahme während der Recherche:** Die Dimension `D0=LZ` desselben Cubes liefert NICHT die historische Reihe, sondern denselben aktuellen SNB-Leitzins wie `snbgwdzid` (Verwechslungsgefahr, da beide Cubes ein `LZ`-Item führen) — die richtige Dimension für die Libor-Periode ist `UG0`/`OG0`.
  - Die beiden Reihen werden **NICHT zu einer nahtlosen Reihe verkettet** — es sind unterschiedliche Instrumente. Jede Instrumentenphase bleibt als eigener, klar gekennzeichneter Abschnitt im Chart erkennbar (Bruchstellen sichtbar markiert, siehe Fussnote unten).
- **⚠️ Offener Rest von V6 — nicht durch Raten zu schliessen, sondern zu melden:** Die dritte Instrumentenphase aus diesem Abschnitt (Diskont-/Lombardsatz, "bis ~2000") konnte in `snboffzisa` NICHT gefunden werden — der Cube führt laut seiner Dimensionsstruktur nur `LZ` (aktueller Leitzins) und `UG0`/`OG0` (Libor-Zielband), keine dritte, ältere Kategorie. Zwei Möglichkeiten, beide ungeprüft: (a) die Diskont-/Lombardsatz-Reihe liegt in einem dritten, noch nicht gefundenen Cube — via SNB-Discovery-Heuristik (Abschnitt 3) weiterzusuchen; (b) die ursprüngliche Annahme "bis ~2000" war zu grosszügig und die tatsächlich verfügbare historische Tiefe ist kürzer. **Vor Produktivschaltung des Leitzins-Overlays zu klären, blockiert aber nicht den Rest von P2** (die zwei gefundenen Cubes belegen bereits zwei von drei Instrumentenphasen lückenlos und ineinandergreifend).
- **Zeitraum (verifizierter Teil):** Januar 2000 (Libor-Zielband) bis heute, lückenlos. Zeitraum vor Januar 2000 (Diskont-/Lombardsatz) offen, siehe oben. Wie bei allen Linien gilt: vor dem Verfügbarkeitsbeginn greift der Nichtexistenz-Zustand aus US 3.16, keine Interpolation.
...
- **Fussnote Pflicht:** Instrumentenbruch — Diskont-/Lombardsatz (bis ~2000, Quelle noch offen) → Libor-Zielband (Januar 2000–Mai 2019, Cube `snboffzisa`, Dimension `UG0`/`OG0`) → SNB-Leitzins (ab 13.06.2019, Cube `snbgwdzid`, Dimension `LZ`). Nicht durchgehend dasselbe Instrument — als Stufenlinie mit sichtbarer Kennzeichnung der Bruchstellen darstellen, nicht glätten oder verketten.
...
| SNB | Geldmenge **M2** (M1/M3 optional erst v2) | monatlich | **Cube `snbmonagg`** (verifiziert 25.08.2026). Daten: `/api/cube/snbmonagg/data/csv/de?dimSel=D0(B),D1(GM2)&fromDate=…&toDate=…`; Struktur: `/api/cube/snbmonagg/dimensions/de` — **ohne `/json`-Segment**. **KORRIGIERT (25.08.2026):** `GM2`=M2 (nicht `GM1`, wie eine frühere Fassung dieses Dokuments ungeprüft aus einem SNB-Doku-Beispiel übernahm — `GM1`=M1). Live über den `/dimensions/de`-Endpoint bestätigt, nicht angenommen (D1-Werte: B=Bargeldumlauf, S0=Sichteinlagen, ET=Einlagen auf Transaktionskonti, GM1=M1, S1=Spareinlagen, **GM2=M2**, T=Termineinlagen, GM3=M3). | Ja |
...
- **Bekannte Einschränkung (verifiziert 25.08.2026): kein serverseitiges conditional GET.** Der Endpunkt liefert zwar einen `Last-Modified`-Header, ignoriert aber `If-Modified-Since` und `If-None-Match` vollständig — jede Prüfung ist ein voller `200`-Download (~190KB), nie ein `304`. Das untergräbt US 1.16 an der wichtigsten Stelle (Bandbreiten-Ersparnis durch bedingte Anfragen). Ersatzlösung: die Pipeline vergleicht den `Last-Modified`-Wert client-seitig NACH dem Download, um zu entscheiden, ob eine Verarbeitung nötig ist. Konsequenz: das Tagesbudget (`maxSuccessfulChecksPerDay`, US 1.16) hat für diese Quelle höheres Gewicht als bei Quellen mit funktionierendem conditional GET, weil jeder Check unabhängig vom Ergebnis volle Bandbreite kostet.

**Fachapp-Heuristik für die Quellensuche (aus V1 gelernt, für V2–V5 verbindliche Suchreihenfolge):** Bei BFS-Themen mit eigener Erhebung existiert häufig eine dedizierte Fachapplikation unter `<kürzel>-app.bfs.admin.ch`, erkennbar am «Rechner»- oder «Online-Applikation»-Link auf der Themenseite bzw. in der Downloads-Rubrik. Deren `global-params.js` (oder vergleichbare Config) verlinkt oft direkt auf die Rohdaten-Endpunkte. **Suchreihenfolge für jede BFS-Quelle:** (1) dedizierte Fachapp, (2) Swiss Stats Explorer/SDMX, (3) STAT-TAB/PxWeb, (4) BFS-Support **nur als allerletztes Mittel, nach Rückfrage beim Betreiber** (Betreiber-Entscheidung 27.08.2026 — nicht eigenmächtig kontaktieren, auch wenn (1)-(3) erschöpft sind). Konkret für V2 bereits gefunden: dieselbe `global-params.js` der LIK-App enthält `kvpiDataUrl` und `itrDataUrl`.

**SNB-Pendant zur Fachapp-Heuristik (gefunden bei V6/Leitzins, 25.08.2026):** Das SNB-Datenportal (`data.snb.ch`) organisiert seine Cubes über **Publikationssets** und **Themenseiten**, nicht über eine zentrale, einfach durchsuchbare Cube-Liste — Cube-Namen erraten (z.B. Vermutungen wie `snbpolzin`, `snbchpolzin`) ist unproduktiv (>20 Fehlversuche bei der Leitzins-Suche). Stattdessen: **Publikationssets durchsuchen**, z.B. `data.snb.ch/de/publishingSet/A` ("Zinssätze und Devisenkurse"), und die dortigen Themenseiten nach dem URL-Muster `data.snb.ch/de/topics/<thema>/cube/<cubename>` abklopfen — das listet die tatsächlich zum Thema gehörenden Cube-IDs auf, statt sie zu vermuten. **Suchreihenfolge für jede SNB-Quelle:** (1) Publikationsset zum Thema identifizieren, (2) Themenseiten-URLs nach `cube/<name>`-Mustern durchsuchen, (3) gefundene Cube-ID über `/api/cube/<name>/dimensions/de` verifizieren (Struktur bestätigen, nicht die Dimension-Werte raten — siehe GM1/GM2-Fund oben), (4) SNB-Support erst danach.
...
- **v2:** Mehrsprachigkeit (FR/IT/EN), Embed-Funktion (iframe für Blogs/fremde Seiten — Reichweiten-Multiplikator), Reallohn-Overlay (präzisiert, Betreiber-Entscheidung 27.08.2026 — siehe unten), ElCom-Strom nur falls je eine konkrete LIK-Schwäche belegt wird, umschaltbare Geldmenge M1/M2/M3 (v1 zeigt nur M2)
  - **Kantonale Zusatzansicht: GESTRICHEN (Betreiber-Entscheidung 27.08.2026).** Widerspricht dem Kernprinzip "ein Wert für die Schweiz" (Abschnitt 1, Nicht-Ziele). Nicht als v2-Kandidat weiterverfolgen.
  - **Reallohn-Overlay: PRÄZISIERT, bleibt v2 (Betreiber-Entscheidung 27.08.2026).** NICHT der offizielle BFS-Reallohnindex — dieser ist bereits mit dem amtlichen LIK bereinigt und wäre damit methodisch inkonsistent zur Trueflation-Eigenkonstruktion. Stattdessen: Nominallöhne (BFS) mit der eigenen Trueflation-Reihe bereinigen, nicht mit dem offiziellen LIK. Zielformulierung für den Content (Beispiel, Zahlen noch zu berechnen): "Offiziell Reallöhne seit 2010 +X% — mit Prämien gerechnet nur +Y%." Macht den Trueflation-Effekt an einer zweiten, alltagsnahen Grösse (Lohn statt nur Index) sichtbar.
