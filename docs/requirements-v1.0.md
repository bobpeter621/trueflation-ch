...
#### 2.2d Status-Übersicht der drei Trueflation-Komponenten (Betreiber-Entscheidung 27.08.2026)

**Fixer Warenkorb: OFFEN, VOR v1 ZU KLÄREN (geändert von "zurückgestellt").** Solange nicht geklärt ist, ob der Warenkorb-Fixierungseffekt überhaupt eine relevante Grössenordnung erreicht, weiss niemand, ob die Methodik in dieser Form Bestand hat — es gibt dafür keine Deadline, aber es ist ein offener Punkt, kein zurückgestelltes Feature. Nächste Schritte (verbindliche Reihenfolge):
1. **LIK-App-Endpunkt auf Gewichte prüfen** — geprüft 27.08.2026: Die gesamte `dam-api.bfs.admin.ch`-LIK-App-State-Struktur (`majorGroupsYearly`, `majorGroupsMonthly`, `additionalClassificationsYearly/-Monthly`) enthält **keine Gewichts-/Anteilsfelder**, nur Indexwerte pro `posId`. Die parallele **ITR-App** (`ds-q-05.02-itr-app-state`) enthält ein `gewichtsProfile`-Feld, das aber **sechs aktuelle Einkommensklassen-Budgetprofile** (Stand 26.07.2026, `budget`/`incomeMin`/`incomeMax` pro Profil) sind — keine historischen Fixierungsjahr-Gewichte (2010/2015/2020). **Weg (a) ist damit negativ**, nicht weiter zu verfolgen.
2. **BFS-Medienmitteilungen (DAM-Publikationssystem) statt Datensätze durchsuchen** — noch nicht durchgeführt (Fokus lag zuerst auf Weg a). Suchbegriffe "Warenkorb", "Gewichtung", "Revision" für die Jahrgänge 2010/2015/2020, PDF-Extraktion akzeptabel (statische Werte, einmalige Beschaffung). **Abbruchkriterium:** erschöpft ohne Gewichte für mindestens ZWEI Fixierungsjahre → melden und stoppen, nicht schätzen.
3. **Wirksamkeitstest VOR Einbau (nie beantwortet):** Sobald echte Gewichte für zwei Fixierungsjahre vorliegen, den fixierten Index gegen den offiziellen verketteten LIK halten, BEVOR eingebaut wird. Trägt der Korbeffekt weniger als ~10% des Prämieneffekts bei (unter ~0.03 pp/Jahr), ist er Rauschen → melden, Betreiber entscheidet dann über die Komponente.
4. **Formel-Reihenfolge, verbindlich:** ERST Preisreihen korrigieren (Miete), DANN Gewichte fixieren und reskalieren (Warenkorb + Prämien) — sonst je nach Implementierung anderer Wert.

**Miet-Korrektur: TEILWEISE GELÖST, Konzeptentscheidung noch offen (27.08.2026 präzisiert).** Frühere Einschätzung "komplett zurückgestellt" war eine Verwechslung — was tatsächlich fehlte, war die Angebots-/Bestandsmieten-Wachstumsdifferenz für eine separate Überschlagsrechnung, nicht die eigentliche Datengrundlage. Diese existiert (acht XLSX-Tabellen "Durchschnittlicher Mietpreis nach Bezugsdauer der Wohnung", Thema 900140) und wurde ausgewertet, siehe `scripts/pipeline/build-rent-correction.mjs`:
- Neumieter-Proxy = Zeile "Neubezug einer mehr als zwei Jahre alten Wohnung" gegen "Total" (methodische Entscheidung, dokumentiert im Skript).
- **Beide Varianten berechnet:** ungewichtet (Delta -0.66pp, 2020-2024) und gewichtet mit dem tatsächlichen Bevölkerungsanteil der Neubezug-Klasse (Delta -0.16pp). **Konzeptfrage an Betreiber weiterhin offen:** welche Variante wird produktiv verwendet.
- **Wesentliche neue Einschränkung:** Die real abrufbaren Quelltabellen (DAM-Assets) liefern nur **2020-2024** (5 Jahrgänge), nicht ab 2010 wie in diesem Dokument angenommen — kein Archivzugang für 2010-2019 gefunden.
- **Meldepflichtiger Widerspruchsbefund:** Eine Kreuzprüfung gegen die Mietpreis-PRO-M2-Tabelle zeigt die GEGENTEILIGE Korrekturrichtung (+1.84pp statt -0.66pp) — absoluter Mietpreis und Preis pro Quadratmeter widersprechen sich in der Richtung. Mögliche, nicht verifizierte Ursache: Neumieter beziehen im Schnitt kleinere Wohnungen. Noch nicht in die Trueflation-Gesamtberechnung integriert.

**Strom: bleibt GESTRICHEN** (unverändert, siehe 2.2c — Doppelzählung belegt, Elektrizität bereits vollständig im LIK enthalten).

**Aktuelle Kernzahl (LIK 5.51% / Trueflation 9.66% Jahresdurchschnitt 2010-2024) ist VORLÄUFIG** — basiert nur auf der Prämienkorrektur, noch nicht auf Warenkorb oder Miete. Nicht nach aussen kommunizieren, bevor diese beiden Komponenten entschieden sind.
